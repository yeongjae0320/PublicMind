...
### 2.1 메인 대시보드 (통합 AI 어시스턴트)
<img src="./screenshots/1_main.png" width="600" alt="메인 대시보드 화면" />
- **3D 커버플로우 UI**: 방대한 공공데이터 카테고리를 직관적이고 입체적으로 탐색할 수 있습니다.
- **음성 인식 비서**: 하단의 마이크 버튼을 통해 자연어로 질문하면 AI가 의도를 파악하여 적합한 서비스로 즉시 안내합니다.

### 2.2 마이페이지 (초개인화 맞춤 분석)
<img src="./screenshots/2_mypage.png" width="600" alt="마이페이지 화면" />
- **프로필 기반 AI 매칭**: 사용자의 나이, 거주지, 직업, 가구 형태를 기반으로 나에게 꼭 맞는 정책과 지원금을 필터링하여 보여줍니다.
- **결과 리포트 제공**: 수많은 공공데이터 중 내가 혜택을 받을 수 있는 알짜 정보만 모아 대시보드 형태로 제공합니다.

### 2.3 글로벌 재난/안전 대시보드
<img src="./screenshots/3_safety.png" width="600" alt="글로벌 재난/안전 화면" />
- **실시간 위험 경보 지도 시각화**: 외교부 해외안전여행 데이터를 연동하여 전 세계 위험 지역을 Leaflet 기반 인터랙티브 지도로 한눈에 파악할 수 있습니다.
- **AI 인사이트 브리핑**: 현재 발령 중인 특별여행주의보 및 출국권고 국가를 AI가 요약하여 리포팅해 줍니다.

### 2.4 아파트 실거래가 분석
<img src="./screenshots/4_realestate.png" width="600" alt="아파트 실거래가 화면" />
- **국토교통부 실거래가 데이터 연동**: 원하는 시/도 및 시/군/구를 선택하여 해당 지역의 최신 아파트 매매 기록을 실시간으로 확인합니다.
- **데이터 시각화**: 평형별 거래 건수를 막대그래프(Bar Chart)로 시각화하여 지역별 부동산 동향을 직관적으로 분석합니다.
