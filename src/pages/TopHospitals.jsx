...
import { MapPin, Search, Clock, Phone, Navigation, Crosshair, Award, CheckCircle, ShieldCheck, HeartPulse, Activity, Star } from 'lucide-react';
import BookmarkButton from '../components/BookmarkButton';
import { fetchTopHospitals } from '../services/healthApi';
import AiInsightCard from '../components/AiInsightCard';

// Custom Kakao Map Component
function KakaoMap({ data, activeId, onMarkerClick, userLocation }) {
    ...
    if (data && data.length > 0) {
      window.kakao.maps.load(() => {
        data.forEach((item) => {
          ...
          const markerColor = '#eab308'; // Gold theme for 1st grade
          ...
          const markerHTML = `
            ...
            ">
              ...
              ">
                <svg width="${isActive ? '18' : '14'}" height="${isActive ? '18' : '14'}" viewBox="0 0 24 24" fill="${isActive ? 'white' : 'none'}" stroke="${isActive ? 'white' : markerColor}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
function TopHospitals() {
  ...
  const itemsPerPage = 3; // Match Health.jsx pagination

  const filters = ['전체', '내과', '외과', '소아청소년과', '이비인후과', '정형외과'];

  useEffect(() => {
    const loadData = async (lat, lng) => {
      setLoading(true);
      try {
        const result = await fetchTopHospitals(lat, lng);
        setData(result);
  ...
  const filteredData = data.filter(item => {
    const matchSearch = item.name.includes(searchTerm) || item.address.includes(searchTerm);
    const matchFilter = activeFilter === '전체' || item.specialties.includes(activeFilter);
    ...
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 'calc(100vh - 120px)' }}>
      ...
      <div style={{ marginBottom: '24px', textAlign: 'center', position: 'relative' }}>
        ...
        <p className="page-subtitle" style={{ fontSize: '1.1rem', margin: '0 auto', maxWidth: '800px', marginBottom: '24px' }}>
          건강보험심사평가원 우수 평가를 받은 내 주변 최고 등급 병원을 확인하세요.
        </p>
          ...
          {filters.map(filter => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s',
                background: activeFilter === filter ? '#eab308' : 'rgba(255,255,255,0.7)',
                color: activeFilter === filter ? 'white' : 'var(--text-secondary)',
                boxShadow: activeFilter === filter ? '0 4px 12px rgba(234, 179, 8, 0.3)' : '0 2px 5px rgba(0,0,0,0.05)'
              }}
            >
              {filter}
            </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '24px', flex: 1, minHeight: '600px' }}>
        ...
        <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden', padding: 0, display: 'flex', flexDirection: 'column', border: '1px solid var(--border-light)' }}>
          ...
          <div style={{ position: 'absolute', top: '24px', left: '24px', right: '24px', zIndex: 10, display: 'flex', gap: '12px' }}>
            ...
            >
              <Crosshair size={24} color="#eab308" />
            </button>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '24px', borderBottom: '1px solid var(--border-light)' }}>
            <h2 style={{ fontSize: '1.2rem', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
              검색 결과 <span className="badge" style={{ background: '#eab308', color: 'white' }}>{filteredData.length}</span>
            </h2>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }} className="custom-scrollbar">
            {loading ? (
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#eab308', textAlign: 'center', padding: '20px' }}>
                <MapPin size={48} style={{ marginBottom: '16px', opacity: 0.5 }} />
                <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{locationError}</span>
              </div>
            ) : currentItems.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-muted)' }}>
                해당 조건의 1등급 병원을 찾을 수 없습니다.
              </div>
            ) : (
              currentItems.map(item => {
                const isActive = activeId === item.id;
                const themeColor = '#eab308';
                const themeShadow = 'rgba(234,179,8,0.4)';
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        <span style={{ 
                          ...
                          background: isActive ? 'rgba(255,255,255,0.2)' : '#fefce8',
                          color: isActive ? 'white' : '#ca8a04',
                          display: 'flex', alignItems: 'center', gap: '4px'
                        }}>
                          <Star size={10} fill="currentColor" /> 1등급 우수
                        </span>
                        <span style={{ 
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div onClick={(e) => e.stopPropagation()} style={{ background: isActive ? 'rgba(255,255,255,0.2)' : 'var(--bg-base)', borderRadius: '50%', display: 'flex', zIndex: 10 }}>
                          <BookmarkButton item={item} type="hospital" title={item.name} subtitle={item.address} link={item.phone ? `tel:${item.phone}` : ''} itemId={item.id} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '12px' }}>
                      {item.specialties.map(spec => (
                        <span key={spec} style={{ background: isActive ? 'rgba(255,255,255,0.2)' : '#f1f5f9', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75rem', color: isActive ? 'white' : '#475569', fontWeight: 600 }}>
                          #{spec}
                        </span>
                      ))}
                    </div>
                    ...
                    {isActive && (
                      <button 
                        onClick={(e) => { e.stopPropagation(); item.phone && window.open(`tel:${item.phone}`); }}
                        style={{ 
